...
QPushButton#blankBtn { background-color: #D32F2F; border: none; padding: 6px 14px; }
QPushButton#blankBtn:checked { background-color: #388E3C; }
...
class ControlWindow(QMainWindow):
    def __init__(self, display_window):
        ...
        # Blank Output Toggle Switch
        self.btn_blank = QPushButton("⬛ Blank Screen")
        ...
        device_layout.addWidget(self.btn_blank)
    ...
    def toggle_blank_screen(self):
        ...
        if self.is_screen_blanked:
            self.btn_blank.setText("🟩 Unblank Screen")
            self.display_window.video_widget.hide()
            self.lbl_preview.clear()
        else:
            self.btn_blank.setText("⬛ Blank Screen")
            # Only unhide the output widget if Deck A has media active
    ...
    def process_video_frame(self, frame: QVideoFrame):
        # Stop sending frames to the preview box if output is blanked
        if self.is_screen_blanked:
            self.lbl_preview.clear()
            return
